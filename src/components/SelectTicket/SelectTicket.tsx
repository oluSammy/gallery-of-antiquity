...
const SelectTicket = (props: Props) => {
  return (
    <div className="border-b pb-3 flex justify-between items-center my-6">
      ...
      <div className={`flex items-center sans ${props.active ? "" : "opacity-40"}`}>
        <p className="mr-2 text-[#474848] font-medium">
          {props.price === 0 ? "Free" : <span>&#x20A6; {formatNumber(props.price)}</span>}
        </p>
        <div className="flex items-center justify-center">
          <button
            className={`flex items-center justify-center h-5 w-5 rounded-full border  cursor-pointer ${props.active ? "" : "cursor-not-allowed"}`}
          ...
          <input
            className={`mx-3 w-12 rounded-md px-2 py-1 text-center ${props.active ? "" : "cursor-not-allowed"}`}
            style={{ border: "0.5px solid #B4B4B4", color: "#B4B4B4" }}
            type="number"
            value={props.value}
            min={0}
          ...
          />
          <button
            className={`${btnClassName} ${props.active ? "" : "cursor-not-allowed"}`}
            style={{ border: "0.5px solid #B4B4B4", color: "#B4B4B4" }}
            onClick={() => {
              if (props.active) {
                props.setValue(props.value + 1);
